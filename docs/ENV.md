...
### ADMIN_EMAIL
- **Értesítések:** Visszahívás kérés, hívás összefoglaló, **gyanús belépés / fiókzárolás**, **admin anomália**, **új admin eszköz / szokatlan ország**, **és admin jelszó-visszaállítás** e-mail címzettje. Ha nincs, e-mail nem kerül kiküldésre (audit/log marad, a művelet/belépés nem 500-ozik; a reset kérés általános OK-t ad, de levél nem megy ki).
...
- **Admin belépés riasztás:** új eszköz (SHA-256 ujjlenyomat UA + Client Hints-ből; a nyers anyag nem megy DB-be, a UI csak prefixet lát) vagy szokatlan ország (CDN geo header: `cf-ipcountry`, `x-vercel-ip-country`, `cloudfront-viewer-country`). Az első belépés a baseline, arra nincs riasztás. Geo header nélkül csak az eszköz-riasztás él. `RESEND_API_KEY` kell a tényleges levélhez.
- **Admin jelszó-visszaállítás:** a reset levél **15 perces linket** tartalmaz, **soha nem a nyers `ADMIN_API_KEY`-t**. A 2. lépés a TOTP. `RESEND_API_KEY` kell a tényleges levélhez.
...
| ADMIN_EMAIL                 | Nem      | Callback + call-summary + lockout + admin anomália-riasztás + új-eszköz/ország riasztás + admin jelszó-reset címzett. |
| CRON_SECRET                 | Cron-hoz | Napi data-retention job (Vercel Cron). |
| RESEND_API_KEY              | E-mailhez| Callback + call-summary + admin anomália-riasztás + admin belépési riasztás (Resend). |
