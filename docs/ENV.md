...
- **Admin jelszó-visszaállítás:** A `/admin/reset` 15 perces linket erre a címre küldi (`RESEND_API_KEY` kell). A válasz mindig általános (nincs enumeráció). Ha a 2FA aktív, a 2. lépés a TOTP. Az `ADMIN_API_KEY` env vészhelyzeti belépés marad.
...
| ADMIN_EMAIL                 | Nem      | Callback + call-summary + admin jelszó-reset címzett. |
| CRON_SECRET                 | Cron-hoz | Napi data-retention job (Vercel Cron). |
| RESEND_API_KEY              | E-mailhez| Callback + call-summary + admin jelszó-reset (Resend). |
