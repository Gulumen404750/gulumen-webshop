...
export default function OrderShippingEditPage() {
  ...
  }, [authChecked, isLoggedIn, orderId, router, t])

  if (!authChecked || (!isLoggedIn && !error)) {
    return (
        ...
        <div className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--card-bg)] p-4 sm:p-6">
          ...
          {!order.canEditShipping ? (
            <div className="rounded-lg border border-[var(--border)] bg-background px-3 py-3 text-sm text-muted">
              ...
              <p className="mt-2">
                <Link href="/profil/rendelesek" className="text-accent hover:underline">
                  {t('orders.title')}
                </Link>
              </p>
            </div>
          ...
          {saved && (
            <p className="text-sm text-muted">
              {t('orders.editPageAdminNotified')}{' '}
              <Link href="/profil/rendelesek" className="text-accent hover:underline">
                {t('orders.title')}
              </Link>
            </p>
