...
import { getProductBySlug, getStockById, getProductName, mockProducts } from '@/lib/data'
        ...
        <div className="space-y-4">
          ...
          >
            {mainImage?.startsWith('/') ? (
              <Image src={mainImage} alt={productName} fill className="object-contain" sizes="(max-width: 1024px) 100vw, 50vw" priority />
            ) : (
              <div className="absolute inset-0 flex items-center justify-center text-muted">{t('product.noImage')}</div>
            )}
          </div>
          {(hasMultipleImages || has360) && (
              ...
              {images.slice(0, 6).map((img, i) => (
                <button
                  ...
                  className={`w-20 h-20 shrink-0 rounded-lg border-2 bg-[var(--card-bg)] relative overflow-hidden transition-colors ${
                    i === mainImageIndex ? 'border-accent ring-2 ring-accent/30' : 'border-[var(--border)] hover:border-accent/50'
                  }`}
                  aria-label={`${productName} ${i + 1}`}
                  aria-pressed={i === mainImageIndex}
                >
                  {img.startsWith('/') && <Image src={img} alt="" fill className="object-cover" sizes="80px" />}
                </button>
              ))}
              {has360 && (
                <button
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4">
            ...
            <button
              ...
              onClick={() => {
                ...
                if (userId) headers['X-User-Id'] = userId
                fetch(`/api/products/${product.id}/like`, { method: 'POST', headers })
                  .then((r) => {
                    if (r.status === 401) {
                      setLiked(prevLiked)
