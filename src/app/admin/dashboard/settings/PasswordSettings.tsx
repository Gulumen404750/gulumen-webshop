...
export default function PasswordSettings() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setNewPassword2('')
      setTotpCode('')
      setMessage('A jelszó elmentve. Elfelejtéskor az e-mailes visszaállítás használható, env-cserére nincs szükség.')
    } catch (e) {
    ...
    <section className="rounded-xl border border-[var(--border)] bg-background p-4 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted mt-1">
          A jelszó az adatbázisban van, kétlépcsősen visszaállítható (e-mail + 2FA). Az{' '}
          <code>ADMIN_API_KEY</code> env vészhelyzeti belépés marad.
        </p>
      </div>
