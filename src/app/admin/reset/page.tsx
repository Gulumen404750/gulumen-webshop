...
function AdminResetForm() {
  ...
  const loginHref = publicAdminUiPathFromBase('/admin/login', readAdminPublicBase())
  ...
  async function handleRequest(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage(
        data?.message || 'Ha a visszaállítás elérhető, elküldtük a linket az admin e-mail címre.'
      )
    } catch {
      ...
      {token ? (
        <form onSubmit={handleConfirm} className="w-full max-w-sm space-y-4">
          ...
          <p className="text-sm text-muted">
            A link 15 percig érvényes. A mentéshez a hitelesítő alkalmazás 6 jegyű kódja is kell. Az
            API kulcsot e-mailben soha nem küldjük.
          </p>
          <div>
          ...
          <p className="text-center text-sm">
            <Link href={loginHref} className="text-muted hover:text-foreground underline">
              Vissza a belépéshez
            </Link>
          </p>
        </form>
        ...
        <form onSubmit={handleRequest} className="w-full max-w-sm space-y-4">
          ...
          <p className="text-sm text-muted">
            A 15 perces linket az admin e-mail címre küldjük. A 2. lépés a hitelesítő kód. A nyers
            API kulcs nem szerepel a levélben.
          </p>
          {error && <p className="text-sm text-red-600 dark:text-red-400">{error}</p>}
