...
describe('checkout API payment methods', () => {
  ...
  it('still blocks two percent coupons before creating a payment', () => {
    ...
    expect(src).toMatch(/'minOrderHuf' in resolved/)
    expect(src).toMatch(/minOrderHuf != null \? \{ minOrderHuf \}/)
    expect(src).toMatch(/secondaryCouponId/)
    expect(src).toMatch(/isFixedCouponDiscount/)
    expect(src).toMatch(/fixedIds/)
    expect(src).toMatch(/points_coupon_stack_disabled/)
    expect(src).toMatch(/isCouponPointsStackBlocked/)
    expect(src).toMatch(/abandonedCart/)
