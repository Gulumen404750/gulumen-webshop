...
const TEST_ACTOR = { id: 'admin', username: 'admin', role: 'owner' as const, bootstrap: true }

describe('/api/admin/password', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    requireAdminPermission.mockResolvedValue({ ok: true, actor: TEST_ACTOR })
  ...
  it('POST rejects when the caller lacks settings:write permission', async () => {
  ...
  it('POST re-signs the session for the acting operator, not the bootstrap actor', async () => {
    const operatorActor = { id: 'op-1', username: 'kata', role: 'support' as const }
    requireAdminPermission.mockResolvedValue({ ok: true, actor: operatorActor })
    const { POST } = await import('@/app/api/admin/password/route')
    const res = await POST(
      new Request('http://localhost/api/admin/password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ newPassword: 'CorrectHorse1', totpCode: '123456' }),
      })
    )
    expect(res.status).toBe(200)
    expect(createAdminSessionToken).toHaveBeenCalledWith(operatorActor)
  })
})
