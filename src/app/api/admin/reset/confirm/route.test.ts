...
describe('POST /api/admin/reset/confirm', () => {
  ...
  it('rejects confirm when TOTP is missing without consuming the token', async () => {
  ...
  it('rejects confirm when TOTP is wrong without consuming the token', async () => {
  ...
  it('sets the password, bumps session epoch, and requires TOTP', async () => {
    ...
    expect(rateLimit).toHaveBeenCalledWith(expect.anything(), { preset: 'adminResetConfirm' })
  })
