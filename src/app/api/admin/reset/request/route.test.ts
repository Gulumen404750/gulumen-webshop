...
describe('POST /api/admin/reset/request', () => {
  ...
  it('always returns the same generic success payload', async () => {
    ...
    const sent = await POST(new Request('http://localhost/api/admin/reset/request', { method: 'POST' }))
    ...
    issueAdminPasswordResetEmail.mockResolvedValue({ issued: false, reason: 'no_password' })
  ...
  it('rate-limits the mailbox without changing the generic wording on other errors', async () => {
    ...
    const { POST } = await import('@/app/api/admin/reset/request/route')
    const res = await POST(new Request('http://localhost/api/admin/reset/request', { method: 'POST' }))
