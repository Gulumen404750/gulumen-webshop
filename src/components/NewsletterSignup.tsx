...
export function NewsletterSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('ok')
        setMessage(data.message || (t('newsletter.success') || 'Feliratkozás sikeres.'))
        setEmail('')
      } else {
        setStatus('error')
        setMessage(data.error || (t('newsletter.error') || 'Hiba történt.'))
      }
    } catch {
      setStatus('error')
      setMessage(t('newsletter.error') || 'Hiba történt.')
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2 max-w-md mx-auto">
      <label htmlFor="newsletter-email" className="sr-only">
        {t('newsletter.emailLabel') || 'E-mail a hírlevélhez'}
      </label>
      <input
        id="newsletter-email"
        type="email"
        ...
        placeholder={t('newsletter.placeholder') || 'e-mail címed'}
      ...
      >
        {status === 'loading' ? (t('newsletter.sending') || 'Küldés...') : (t('newsletter.submit') || 'Feliratkozom')}
      </button>
      {message && (
