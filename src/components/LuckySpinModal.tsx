...
export function LuckySpinModal({ isOpen, onClose, data, onSpin, spinning }: Props) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-lg rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] shadow-2xl p-6 max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <h2 className="font-heading text-xl font-bold text-foreground mb-1 pr-8">
          {isActive ? t('luckySpin.offerActive') : t('luckySpin.title')}
        </h2>
        <p className="text-sm text-muted mb-5">
          {isActive ? t('luckySpin.discountHint') : t('luckySpin.subtitle')}
        </p>
        ...
        {!isEligible && (
          <div className="flex flex-col items-center gap-4 py-6">
            <div
              className="relative w-40 h-40 rounded-full border-4 border-[var(--border)] flex items-center justify-center opacity-50 grayscale"
              style={{ background: 'conic-gradient(from 0deg, #9ca3af 0deg 360deg)' }}
            >
              <div className="w-16 h-16 rounded-full bg-[var(--card-bg)] border-2 border-[var(--border)] flex items-center justify-center text-2xl">
                🎡
              </div>
            </div>
        ...
        {isEligible && canSpin && !isActive && (
          <div className="flex flex-col items-center gap-4 py-4">
            <div
              className={`relative w-40 h-40 rounded-full border-4 border-accent/30 flex items-center justify-center ${
                animating || spinning ? 'animate-spin-slow' : ''
              }`}
              style={{
                background: 'conic-gradient(from 0deg, var(--accent) 0deg 45deg, #f59e0b 45deg 90deg, #ec4899 90deg 135deg, #8b5cf6 135deg 180deg, var(--accent) 180deg 225deg, #f59e0b 225deg 270deg, #ec4899 270deg 315deg, #8b5cf6 315deg 360deg)',
              }}
            >
              <div className="w-16 h-16 rounded-full bg-[var(--card-bg)] border-2 border-[var(--border)] flex items-center justify-center text-2xl">
                🎡
        ...
        {isActive && displayData.spin && (
          <>
            <p className="text-sm font-semibold text-accent mb-3">{t('luckySpin.offerActive')}</p>
            <div className="flex items-center justify-between mb-4 p-3 rounded-xl bg-accent/10 border border-accent/20">
              <span className="text-sm font-medium text-foreground">{t('luckySpin.expiresIn')}</span>
              ...
              {displayData.spin.products.map((p) => (
                <li key={p.id}>
                  ...
                  >
                    ...
                    <p className="p-2 text-xs font-medium text-foreground line-clamp-2">{p.name}</p>
                  </Link>
                </li>
              ))}
            </ul>

            <p className="text-xs text-muted">{t('luckySpin.pointsBonusHint')}</p>
          </>
        )}
