 ...
 * Az első belépés a baseline (nincs e-mail). Hiba / hiányzó ADMIN_EMAIL: csak log, nincs 500.
 */
...
async function sendLoginAlert(
  ...
  if (!to) {
    logger.warn(
      { fingerprintPrefix: fingerprintPrefix(signals.fingerprint), country: signals.countryCode },
      'admin login fingerprint alert skipped: ADMIN_EMAIL missing'
    )
    return false
  }
  const sent = await sendMail({
    ...
    text: buildAdminLoginAlertText({ decision, signals }),
  })
  if (!sent.ok) {
  ...
  if (sent.skipped) {
    logger.warn('admin login fingerprint alert skipped: RESEND_API_KEY missing')
    return false
  }
  return true
}
  ...
  if (shouldAlertAdminLogin(decision)) {
    ...
    await logAdminAction({
      ...
      details: {
        ...
        countryCode: signals.countryCode,
        fingerprintPrefix: fingerprintPrefix(signals.fingerprint),
        alerted,
      },
    })
  }
